...
from mnist_cnn_gan.aee.keras_adversarial.progress_callback import ProgressCallback
...
from mnist_cnn_gan.aee.keras_adversarial.legacy import l1l2

import keras.backend as K
import pandas as pd
import numpy as np
from mnist_cnn_gan.aee.keras_adversarial.image_grid_callback import ImageGridCallback

from mnist_cnn_gan.aee.keras_adversarial import AdversarialModel, fix_names, n_choice
from mnist_cnn_gan.aee.keras_adversarial import AdversarialOptimizerSimultaneous, normal_latent_sampling
from mnist_cnn_gan.aee.keras_adversarial.examples.mnist_utils import mnist_data
...
def model_generator(latent_dim, input_shape, hidden_dim=512, reg=lambda: l1l2(1e-7, 0)):
    return Sequential([
        ...
        Dense(hidden_dim, name="generator_h1", input_dim=latent_dim, kernel_regularizer=regularizers.l2(0.01)),
...
def example_aae(path, adversarial_optimizer):
    ...
    l = xtrain.shape[0]
    xtrain = np.delete(xtrain, np.s_[l-50001:l-1], 0)
    print(xtrain.shape)
    print(xtest.shape)
    ...
    history = model.fit(x=xtrain, y=y, validation_data=(xtest, ytest), callbacks=[generator_cb, autoencoder_cb, progress_cb],
