...
def parse_arg():
    ...
    parser.add_argument(
        '--model', type=str, default='/tmp/my_frozen_graph.pb', help='Pre-trained frozen model.')

    parser.add_argument(
        '--labels', type=str, default='/tmp/conv_labels.txt', help='Path to the file containing word labels.')
...
def start_loop():
    ...
    while True:
        ...
        record_audio(duratio_in_sec=2, out_file_name='/tmp/audio_rec.wav', count=count)

        wav_data = load_audio_bytes('/tmp/audio_rec.wav')
        start_time = time.time()
